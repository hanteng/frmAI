...
作為**符號流 AI**的早期代表，此類系統與**專家系統**類似，其核心思想是將人類專家的知識符號化、規則化，並以**符號比對**的方式進行推論與回應。由於自動對話系統通常依賴手動編寫的**知識庫**與**推理引擎**，因此在封閉、有明確定義的場景下表現較為出色。
...
- 🟢 **1966 年：ELIZA** － 作為早期**符號流 AI** 應用的代表，證明了透過簡單的**模式匹配**能模擬對話，但缺乏真實理解，理由是這種輸入符號與預設規則的模式匹配，缺少基於形式邏輯的複雜**演繹推理**（deductive reasoning），而這演繹推理正是專家系統的核心。
...
- 回憶**符號流 AI**（Symbolic AI）[第參章 🏛️](03----symbolic_ai.zh-hant)的其它條目，評估自己是否能說明**自動對話系統**與它們的關係，如下所述：
    ...
    - **🏛️🎁🧠 [專家系統](03-03-expert_systems.zh-hant)**：早期的自動對話系統可以被視為一種基於模式匹配，但缺少複雜**演繹推理**（deductive reasoning）的專家系統。它們沒有真正的理解能力，而是依賴預先設定好的「如果...則...」腳本來回應，本質上是為了特定目的而設計的「對話專家」。
