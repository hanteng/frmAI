...
- ⮦🚦 探究 [第肆篇 🌀](04----statistical_ai.zh-hant) **統計流 AI**（Statistical AI）的其它條目，評估自己可不可以說明`機器學習模型`和它們的關係，如下所述：
	- **🌀🎲🌿 [機率性關聯](04-01-probabilistic_association.zh-hant)**：`機器學習模型`型透過訓練學習輸入與輸出之間的`機率性關聯`。    
