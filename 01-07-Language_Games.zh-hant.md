...
- 符號主義
---
`語言賽局`（又稱 `語言遊戲`，Language Games）是路德維希・維根斯坦（Ludwig Wittgenstein）提出用以重新界定「語言的意義」的概念，他主張：

> 🗣️🎲 「一個詞的意義，就是它在語言中的用法。」  
> ——《哲學探究》§43

`語言賽局`是其晚期哲學的核心概念。維根斯坦的「語言賽局」與圖靈的「模仿遊戲」皆以遊戲作為框架，但前者強調**語言使用**——嵌入於具體**生命形式**（Forms of Life）中，並具有**規則結構**的語言活動。

語言因此不只是靜態的符號編碼或心智表徵系統，而是一種動態、參與性的社會實踐。詢問、道歉、開玩笑、講故事——每一種都承載隱含的**社會期待**、**利害交換**或**博弈格局**，構成互動中的**賽局**。
...
語言賽局就像一場社會性遊戲，其意義的產生，來自**參與**、**規則遵循**與**互動目的**的動態交織。
...
圖靈的「模仿遊戲」與維根斯坦的「語言賽局」都以「遊戲」作為比喻框架，但兩者的哲學意涵截然不同，反映了對「意義」和「智能」的根本分歧。
...
相對地，維根斯坦的語言賽局則深植於語言的**社會性**與**脈絡性**。它強調語言的意義不是來自於符號本身，而是來自於人們在特定社會生活中的實際使用與互動。

儘管維根斯坦未曾直接探討人工智慧，其語言賽局框架為理解大型語言模型提供視角：大型語言模型在**模擬遊戲**中表現出色，但其生成的內容是否真正觸及了人類**語言賽局**的深層意義與生命形式？

### 📦🎁 大型語言模型的使用

從維根斯坦「意義生成來自使用」的語言賽局觀點來看，大型語言模型的運作呈現出以下特性：

- **生成能力**仰賴對大量語料的「使用」，其成果也必須被應用到「**實際參與**」中才有意義。 
- 生成結果必須涉及到三要素（參與者、規則、及互動目標）的「實際使用」，以觸及人類「生命形式」的脈絡。 
- 模型在直接或間接參與使用脈絡時，其行為突顯了語言賽局的兩大核心： **遵循規則** 與 **家族相似性** 
...
* 🤖 傳統 AI [自動對話系統](03-02-automatic_dialogue_systems.zh-hant)
* 🧠 當代 [LLM 聊天機器人] 
